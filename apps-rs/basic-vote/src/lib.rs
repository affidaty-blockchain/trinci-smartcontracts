...
fn add_vote(ctx: AppContext, args: VoteArgs) -> WasmResult<Value> {
    ...
    if burned.contains(ctx.caller) {
        return Err(WasmError::new("the caller has already voted"));
    }
    ...
    // Add the caller to the burned list.
...
mod tests {
    ...
    const POLL_STATION_ID: &str = types::tests::POLL_STATION_ID;
    ...
    fn adding_vote_duplicate() {
        ...
        assert_eq!(err.to_string(), "the caller has already voted");
    }
